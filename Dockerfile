...
RUN GOOS=linux go build -ldflags '-linkmode=external' $BUILDFLAGS -v -o aws-fix-service
...
COPY --from=builder /src/aws-fix-service/aws-fix-service /aws-fix-service
